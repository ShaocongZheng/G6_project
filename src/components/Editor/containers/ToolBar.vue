...
<template>
  <div class="tool-bar"
    ...
    <template v-for="(type, typeIndex) in Object.keys(toolMap)">
      <ToolBox mode="horizontal"
               :key="typeIndex"
               :class="type">
        <template v-for="(item, index) in toolMap[type]">
          <ToolItem v-if="item.type === 'text'"
            ...
            <template v-slot:label>
              <XIcon v-if="item.icon"
                     :type="item.icon"></XIcon>
              <span v-else>{{ $t(item.lang) }}</span>
            </template>
          </ToolItem>
          <ToolItem v-if="item.type === 'dropdown-color-picker'"
                    :key="'tool_' + type + '_item_' + index"
                    :title="$t(item.lang)"
                    :active="item.active"
                    :disabled="item.disabled"
            ...
            <template v-slot:label>
              ...
              <template v-else>
                <XColorPicker v-model="formData[item.name]"
          ...
          <ToolItem v-if="item.type === 'dropdown-list'"
                    :key="'tool_' + type + '_item_' + index"
                    :title="$t(item.lang)"
                    :active="item.active"
                    :disabled="item.disabled"
            ...
            <template v-slot:label>
              <template v-if="item.disabled">
                <div style="margin: 0 3px;">
                  ...
                  <template v-else>
                    <XIcon v-if="item.children[item.selected].icon"
              ...
              <template v-else>
                <Dropdown trigger="click"
                  ...
                  <DropdownMenu slot="list">
                    <DropdownItem v-for="(child, childIndex) in item.children"
                                  :key="childIndex"
                                  :name="childIndex"
                      ...
                      <template v-if="child.icon">
                        <XIcon :type="child.icon"
                               :title="$t(child.lang)"
                               :style="child.style"></XIcon>
                      </template>
                      <template v-else>
          ...
          <ToolItem v-if="item.type === 'normal'"
                    :key="'tool_' + type + '_item_' + index"
                    :title="$t(item.lang)"
                    :active="item.active"
                    :disabled="item.disabled"
